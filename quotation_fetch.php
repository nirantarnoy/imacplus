...
//    $sub_array[] = '<p style="font-weight: ;text-align: left">'.$row['code'].'<input type="hidden" class="tool-code" value="'.$row['id'].'"></p>';
    ...
    $sub_array[] = '<p style="font-weight: ;text-align: left">'.getCustomerName($row['customer_id']).'</p>';
...
//    $sub_array[] = '<p style="font-weight: bold;text-align: left">'.date('d/m/Y',strtotime($row['trans_date'])).'</p>';
    $sub_array[] = '<a class="btn btn-secondary" href="salecreate.php?update_id='.$row['id'].'"><i class="fas fa-edit"></i> แก้ไข</a><span> </span><div class="btn btn-danger" data-id="'.$row['id'].'" onclick="recDelete($(this))"><i class="fas fa-trash-alt"></i> ลบ</div>';
