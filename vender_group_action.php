...
if (isset($_POST['vender_group_code'])) {
    $code = $_POST['vender_group_code'];
}
if (isset($_POST['vender_group_name'])) {
    $name = $_POST['vender_group_name'];
}
...
if ($action == 'create') {
    $created_at = time();
    $created_by = $userid;
    $sql = "INSERT INTO vender_group(code,name,description,status)
            VALUES('$code','$name','$description','$status')";
    if ($result = $connect->query($sql)) {
        $_SESSION['msg-success'] = 'บันทึกข้อมูลเรียบร้อยแล้ว';
        header('location:vender_group.php');
    }
}
...
//        echo $status;return;
        $created_at = time();
        $created_by = $userid;
        $sql2 = "UPDATE vender_group SET code='$code',name='$name',description='$description',status='$status' WHERE id='$id'";
...
if($action == 'delete'){
    if($delete_id > 0){
        $sql3 = "DELETE FROM vender_group WHERE id='$delete_id'";
