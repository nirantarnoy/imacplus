$(function () {
  ...
  QUnit.test('shoud add toast message', function (assert) {
    ...
    assert.ok($(toast).parent().hasClass('position-tr'))
  ...
  QUnit.test('should add toast message based on existing HTML element', function (assert) {
    ...
    assert.ok($(toast).hasClass('showing'))
    assert.ok($(toast).parent().hasClass('position-br'))
  })
  ...
  QUnit.test('should prevent adding toaster', function (assert) {
    ...
    $(document).on('add.ace.toaster', function (ev) {
  ...
  QUnit.test('should prevent removing toast messages when they are in "top-right" position', function (assert) {
    ...
    $(document).on('clear.ace.toaster', function (ev, info) {
      if (info.placement === 'tr' || info.placement === 'all') ev.preventDefault()
    })
    ...
    setTimeout(function () {
      setTimeout(function () {
        ...
        setTimeout(function () {
          ...
          $(document).off('clear.ace.toaster')
        }, 500)
      }, 500)
    }, 500)
  })
