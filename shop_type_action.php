...
if (isset($_POST['shop_type_name'])) {
    $name = $_POST['shop_type_name'];
}
...
if ($action == 'create') {
    $created_at = time();
    $created_by = $userid;
    $sql = "INSERT INTO shop_type(name,description,status,created_at,created_by)
...
//        echo $status;return;
        $created_at = time();
        $created_by = $userid;
        $sql2 = "UPDATE shop_type SET name='$name',description='$description',status='$status',updated_at='$created_at',updated_by='$created_by' WHERE id='$id'";
...
if($action == 'delete'){
    if($delete_id > 0){
        $sql3 = "DELETE FROM shop_type WHERE id='$delete_id'";
