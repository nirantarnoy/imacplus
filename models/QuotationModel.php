...
function getQuotationAmountByWork($connect, $work_id)
{
    ...
    if ($work_id > 0) {
        $query = '';
        $query = "SELECT SUM(t1.line_total) as quotation_amt FROM quotation_line as t1 INNER quotation as t2 ON t1.quotation_id = t2.id WHERE t2.workorder_id='$work_id'";
...
function getOuotationLastNo($connect)
{
    $query = "SELECT MAX(quotation_no) as code FROM quotation WHERE quotation_no <>''";
    $statement = $connect->prepare($query);
    $statement->execute();
    $result = $statement->fetchAll();
    $filtered_rows = $statement->rowCount();
