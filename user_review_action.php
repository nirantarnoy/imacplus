...
if ($action == 'create') {
    ...
    $created_at = time();
    $created_by = $userid;
    $sql = "INSERT INTO user_review(name,photo,status)
            VALUES('$name','$filename','$status')";
    if ($result = $connect->query($sql)) {
        if($filename != '' && $file_tmp != null){
            move_uploaded_file($file_tmp, "uploads/user_review/" . $filename);
        }
...
//        echo $status;return;
        $created_at = time();
        $created_by = $userid;
        $sql2 = "UPDATE user_review SET name='$name' WHERE id='$id'";
...
if($action == 'delete'){
    if($delete_id > 0){
        $sql3 = "DELETE FROM user_review WHERE id='$delete_id'";
