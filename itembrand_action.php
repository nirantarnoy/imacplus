...
if (isset($_POST['item_brand_name'])) {
    $name = $_POST['item_brand_name'];
}
...
//        echo $status;return;
        $created_at = time();
        $created_by = $userid;
        $sql2 = "UPDATE item_brand SET name='$name',description='$description',status='$status',updated_at='$created_at',updated_by='$created_by' WHERE id='$id'";
...
if($action == 'delete'){
    if($delete_id > 0){
        $sql3 = "DELETE FROM item_brand WHERE id='$delete_id'";
